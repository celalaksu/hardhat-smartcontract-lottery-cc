    ...
    : describe("Raffle Unit Tests", async function () {
          ...
          describe("fulfillRandomWords", function () {
              ...
              it("picks a winner, resets the lottery, and sends money", async () => {
                  ...
                  // perfomUpkeep ( mock being Chainlink Keepes)
                  ...
                  await new Promise(async (resolve, reject) => {
                      raffle.once("WinnerPicked", async () => {
                          ...
                          try {
                              ...
                              // Herşeyin sıfırlandığını kontrol edeceğiz. Çünkü kazanan belirlendi
                              // ve yeni çekiliş süreci başlıyor
                              assert.equal(numPlayers.toString(), "0")
                              assert.equal(raffleState.toString(), "0")
                              assert(endingTimeStamp > startingTimeStamp)
                              // Kazanana ödeme yapılıp yapılmadığından emin olmak için
                              assert.equal(
                      ...
                      // Mocking Chainlink Keeppers an Chainlink VRF
                      const tx = await raffle.performUpkeep([])
                      const txReceipt = await tx.wait(1) // 1 block oluşana kadar bekle
                      const winnerStartingBalance = await accounts[1].getBalance()
                      // fullfillRandomWords V2Mock contratından çağrıldığını unutmayalım.
                      // Ve bu fonksiyonun simulasyonu yapılarak contracttaki fonk. içindeki
                      // değişkenler kontrol edilecektir. ( sıfırlanacaklar )
                      await vrfcoordinatorV2Mock.fulfillRandomWords(
                          // Buradaki fulfillRandomsWords çağrıldığında  "WinnerPicked" olayı
                          // yayınlanacaktır. Ve raffle.once("WinnerPicked") listener ı olayı
